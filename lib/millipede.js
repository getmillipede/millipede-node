...
Millipede.prototype._getVerticalHead = function () {
    ...
    if (this.reverse) {
        return leftPad + PADDING[LAST_PADDING - this.position % PADDING_LENGTH] + '  ╔⊙ ⊙╗';
    }
    return leftPad + PADDING[this.position % PADDING_LENGTH] + '  ╚⊙ ⊙╝';
};
...
Millipede.prototype._getVerticalBody = function () {
    ...
    return Array.apply(null, Array(this.size)).map(function (val, index) {
        ...
        if (self.reverse) {
            part += PADDING[LAST_PADDING - (self.size - 1 - index + self.position) % PADDING_LENGTH];
            part += '╔═(███)═╗';
        }
        else {
            part += PADDING[(index + self.position) % PADDING_LENGTH];
            part += '╚═(███)═╝';
        }
...
Millipede.prototype.getLargestBodyPart = function () {
    ...
    if (this.size < PADDING_LENGTH) {
        usedPadding = PADDING.slice(0, this.size);
    }

    var largest = Math.max.apply(Math, usedPadding.map(function (padding) {
...
Millipede.prototype.toString = function () {
    ...
    if (this.reverse) {
        output += this._getVerticalBody() + '\n' +  this._getVerticalHead();
    }
    else {
        output += this._getVerticalHead() + '\n' +  this._getVerticalBody();
    }
